...
Sahara Pramaan is an independent, accessible certificate-support service concept. It is designed to feel calm, complete, and service-oriented while remaining fully honest that its data, login credentials, fingerprint result, liveness result, family answer, support locations, reminders, and confirmation exports are synthetic.

The product does not connect to government, UIDAI, India Post, banks, pension authorities, biometric providers, messaging providers, real identity systems, or live location services. The synthetic boundary is a product requirement, not a temporary error state.
...
| Local validation | `pnpm check`, `pnpm test` — 5 files / 11 tests passed; `pnpm build:vercel` passed. |
