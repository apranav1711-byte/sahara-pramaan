...
| 2 | Open the comfort menu once. | Large type, high contrast, all ten supported Indian languages, read-aloud, stop-reading, disclosure, and reset controls are visibly available. |
...
8. Show the synthetic confirmation page with its PDF/share actions, then open the support locator. Explain that live Google Places results appear only when the provider path is available; otherwise the result is clearly labeled synthetic. Finish with one accessibility control.
