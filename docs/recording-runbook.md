...
This runbook is for the synthetic public prototype only. It is designed to make a clean two-minute product walkthrough repeatable without relying on random verification outcomes.
...
| 3 | Choose **Reset synthetic session** or use the landing-page quick-start panel. | Any earlier family-assist completion state is cleared. |
...
4. Reset the synthetic session, return to `DEMO-FAIL`, and choose **Ask family to help**.
...
8. Show the synthetic confirmation card, use **Save image** or **Download PDF** if useful, optionally print the card, then show one accessibility control.

## Critical disclosure phrases

Use these phrases or equivalent wording during the walkthrough:

> “This prototype uses a deterministic alternate verification path so the available options are easy to understand.”
...
If a previous state appears, use **Reset synthetic session**. If camera permission is unavailable, use the visible **Continue without a camera** path. If the family share link appears expired, reset the session and create a fresh link from the pensioner’s alternate verification options. If a browser blocks a copy action, copy the displayed value manually.
