...
These two Edge Functions provide the synthetic persistence layer for the public Sahara Pramaan prototype:
...
`verify_jwt` is intentionally disabled for these functions because the public recording flow must be able to open a family-assist link without a Supabase account. This is acceptable only for the explicitly synthetic prototype boundary. A production implementation must replace this arrangement with audited authorization, expiring signed links, rate limits, abuse monitoring, and a real consent and identity-assurance design.
